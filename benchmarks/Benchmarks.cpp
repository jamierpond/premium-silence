...
TEST_CASE ("Boot performance")
{
    ...
    (Catch::Benchmark::Chronometer meter)
    {
        ...
        std::vector<Catch::Benchmark::storage_for<PremiumSilenceAudioProcessor>> storage (size_t (meter.runs()));
    ...
    {
        auto gui = juce::ScopedJuceInitialiser_GUI {};
        std::vector<Catch::Benchmark::destructable_object<PremiumSilenceAudioProcessor>> storage (size_t (meter.runs()));
        for (auto& s : storage)
            s.construct();
    ...
    (Catch::Benchmark::Chronometer meter)
    {
        ...
        PremiumSilenceAudioProcessor plugin;
